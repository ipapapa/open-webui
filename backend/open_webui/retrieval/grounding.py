...
def calculate_query_document_relevance(
    ...
    try:
        ...
        log.debug(
            f"Grounding validation: {len(valid_indices)}/{len(document_embeddings)} documents passed threshold {threshold}"
        )
...
def apply_grounding_step(
    ...
    try:
        ...
        log.debug(
            f"Generating embeddings for {len(document_texts)} documents in grounding step"
        )
        ...
        valid_scores = (
            [relevance_scores[i] for i in valid_indices] if valid_indices else []
        )
        average_confidence = np.mean(valid_scores) if valid_scores else 0.0

        if filtered_count > 0:
            log.info(
                f"Grounding step filtered out {filtered_count} documents below threshold {threshold}"
            )
